...
//  get the analysis values
  ...
  melBands = audioAnalyzer.getValues(MEL_BANDS, 0, .2);
}
...
void ofApp::draw(){
  ...
  for (unsigned int i = 0; i < melBands.size(); i += 2){
    shader.begin();
    shader.setUniform1f("u_time", ofGetElapsedTimef());
    shader.setUniform2f("u_resolution", ofGetWidth(), ofGetHeight());
    shader.setUniform1f("u_sound", rms_l * - (.2 * i));
...
//    ofDrawSphere(ofGetWidth() / 2, ofGetHeight() / 2, 0, radius);
